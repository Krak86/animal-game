...
// Emoji to codepoint mapping for all 68 emojis needed
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(url, (response) => {
      if (response.statusCode !== 200) {
        reject(new Error(`Failed to download ${emoji} (${codepoint}): HTTP ${response.statusCode}`));
        return;
      }
...
async function downloadAll() {
  console.log(`Starting download of ${Object.keys(EMOJI_CODEPOINTS).length} Twemoji SVG files...\n`);
  ...
  for (const [emoji, codepoint] of emojis) {
    try {
      ...
      console.error(`✗ Failed to download ${emoji} (${codepoint}): ${error.message}`);
      failed++;
    }
  }
